...
#simultion 1###################################
phi1 = tran_AR2(module[1,1], (2/1000)*2*pi)
phi2 = tran_AR2(module[2,1], (8/1000)*2*pi)
phi3 = tran_AR2(module[3,1], (15/1000)*2*pi)
...
#plots in the paper
peri_true = matrix(0, 3, 1000)
for(i in 1:3){
  peri_true[i,]= scale(abs(fft(sim$latent[i,,5])/sqrt(1000))^2)
}
peri_est = matrix(0, 3, 1000)
for(i in 1:3){
  peri_est[i,]= scale(abs(fft(reconst$latent[i,,1])/sqrt(1000))^2)

}

temp <- fastICA(t(Signals), 3)$S
peri_est_ica = matrix(0, 3, 1000)
for(i in 1:3){
  peri_est_ica[i,]= scale(abs(fft(temp[,i])/sqrt(1000))^2)
}
...
lines(peri_est_ica[1,1:50], col=c(3), lwd=.5)
...
lines(peri_est_ica[2,1:50], col=c(3), lwd=.5)
...
lines(peri_est_ica[3,1:50], col=c(3), lwd=.5)
...
x = 1:nrow(peri_multi[,1:20])
y = (1:ncol(peri_multi[,1:20]))
temp = (peri_multi[,1:20])
